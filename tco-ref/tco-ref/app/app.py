...
discount_rate = st.session_state.get('discount_rate', 4.0) / 100.0
energy_inflation = st.session_state.get('energy_inflation', 2.0) / 100.0
opex_inflation = st.session_state.get('opex_inflation', 1.5) / 100.0
...
w_home = st.session_state.get('w_slider_a', 90) / 100.0
w_work = st.session_state.get('w_slider_b', 5) / 100.0
w_public = st.session_state.get('w_slider_c', 5) / 100.0
phev_share_elec = st.session_state.get('phev_share_elec', 50) / 100.0
...
with st.expander("🔧 Paramètres avancés"):
    st.caption("Personnalisez tous les paramètres de calcul pour affiner votre analyse TCO")
    
    st.markdown("### ⚙️ Coûts et consommations par technologie")
    ...
    for class_key, class_label in [("petite", "Petites voiture"), ("moyenne", "Classe moyenne"), 
        ...
        with col1:
            ...
            st.number_input(
                ...
                min_value=0.0, 
                max_value=200000.0, 
                value=float(bev_defaults['purchase_price']),
                step=1000.0,
                key=bev_price_key
            )
            ...
            st.number_input(
                ...
                min_value=0.0, 
                max_value=50.0, 
                value=float(bev_defaults['consumption_elec_kwh_per_100']),
                step=0.1,
                key=bev_elec_key
            )
        ...
        with col2:
            ...
            st.number_input(
                ...
                min_value=0.0, 
                max_value=200000.0, 
                value=float(ice_defaults['purchase_price']),
                step=1000.0,
                key=ice_price_key
            )
            ...
            st.number_input(
                ...
                min_value=0.0, 
                max_value=30.0, 
                value=float(ice_defaults['consumption_fuel_l_per_100']),
                step=0.1,
                key=ice_fuel_key
            )
        ...
        with col3:
            ...
            st.number_input(
                ...
                min_value=0.0, 
                max_value=200000.0, 
                value=float(phev_defaults['purchase_price']),
                step=1000.0,
                key=phev_price_key
            )
            ...
            st.number_input(
                ...
                min_value=0.0, 
                max_value=30.0, 
                value=float(phev_defaults['consumption_fuel_l_per_100']),
                step=0.1,
                key=phev_fuel_key
            )
            ...
            st.number_input(
                ...
                min_value=0.0, 
                max_value=50.0, 
                value=float(phev_defaults['consumption_elec_kwh_per_100']),
                step=0.1,
                key=phev_elec_key
            )
    ...
    discount_rate_pct = st.number_input("Taux d'actualisation r (%)", 0.0, 15.0, discount_rate * 100, step=0.5, key="discount_rate")
    energy_inflation_pct = st.number_input("Inflation énergie (%/an)", -5.0, 50.0, energy_inflation * 100, step=0.5, key="energy_inflation")
    opex_inflation_pct = st.number_input("Inflation OPEX (%/an)", -5.0, 20.0, opex_inflation * 100, step=0.5, key="opex_inflation")
    st.checkbox("Pneus ×2 (méthodo)", include_tires_x2, key="include_tires_x2")
    st.checkbox("Maintenance règle 7/6 → 8 ans (placeholder)", apply_maint_7_over_6, key="apply_maint_7_over_6")
    
    st.divider()
    
    st.markdown("### ⚡ Prix de l'énergie")
    st.number_input("Prix carburant (CHF/L)", 0.0, 5.0, fuel_price, step=0.01, key="fuel_price")
    colh, colw, colp = st.columns(3)
    with colh:
        st.number_input("Élec. Maison (CHF/kWh)", 0.0, 1.0, elec_home, step=0.01, key="elec_home")
    with colw:
        st.number_input("Élec. Travail (CHF/kWh)", 0.0, 1.0, elec_work, step=0.01, key="elec_work")
    with colp:
        st.number_input("Élec. Déplacement (CHF/kWh)", 0.0, 2.0, elec_public, step=0.01, key="elec_public")
    
    st.divider()
    
    st.markdown("### 🔌 Profil de recharge BEV/PHEV")
    ...
    w_home_new, w_work_new, w_public_new = three_sliders_sum_to_100(
        "Maison", "Travail", "Déplacement", default_a=w_home, default_b=w_work, default_c=w_public, key_prefix="w"
    )
    ...
    st.markdown("### 🔋 PHEV: Part électrique vs thermique")
